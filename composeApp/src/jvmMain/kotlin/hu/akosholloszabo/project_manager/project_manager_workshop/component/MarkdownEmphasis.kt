...
@Composable
fun MarkdownEmphasis(text: AnnotatedString, modifier: Modifier = Modifier) {
    // Italic styling is applied by the AnnotatedString provider normally; if not, we fallback.
...
@Composable
fun MarkdownEmphasisPreviewLight() {
    val ann = AnnotatedString("This is emphasized text (italic)")
    hu.akosholloszabo.project_manager.project_manager_workshop.AppTheme(darkTheme = false) {
        Surface(color = MaterialTheme.colorScheme.background, modifier = Modifier.fillMaxSize()) {
            MarkdownEmphasis(text = ann, modifier = Modifier.padding(16.dp))
        }
    }
}
...
@Preview(showBackground = true, widthDp = 360, heightDp = 160)
@Composable
fun MarkdownEmphasisPreviewDark() {
    val ann = AnnotatedString("This is emphasized text (italic)")
