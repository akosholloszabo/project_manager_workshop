...
class NoteRepositoryImpl : NoteRepository {
    ...
    override suspend fun getAll(): List<Note> = dbQuery {
        NotesTable.selectAll().map { it.toNote() }
    }

    override suspend fun getById(id: Int): Note? = dbQuery {
        NotesTable.select { NotesTable.id eq id }
            .map { it.toNote() }
            .singleOrNull()
    }

    override suspend fun create(payload: NotePayload): Note = dbQuery {
    ...
    override suspend fun update(id: Int, payload: NotePayload): Boolean = dbQuery {
