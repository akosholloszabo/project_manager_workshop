...
class ProjectRepository {
    ...
    suspend fun getAll(): List<Project> = dbQuery {
        ProjectsTable.selectAll().map { it.toProject() }
    }

    suspend fun getById(id: Int): Project? = dbQuery {
        ProjectsTable.select { ProjectsTable.id eq id }
            .map { it.toProject() }
            .singleOrNull()
    }

    suspend fun create(payload: ProjectPayload): Project = dbQuery {
    ...
    suspend fun update(id: Int, payload: ProjectPayload): Boolean = dbQuery {
